...
test('uses action URL, project, org, and workspace environment defaults', async () => {
  const prior = {
    ...
    org: process.env.RAILGRID_ACTIONS_ORG,
    workspace: process.env.RAILGRID_ACTIONS_WORKSPACE,
  };
  try {
    process.env.RAILGRID_ACTIONS_BASE_URL = 'https://hub.example/services/providers/app-studio';
    process.env.RAILGRID_PROJECT = 'env-project';
    process.env.RAILGRID_ACTIONS_ORG = 'org-from-env';
    process.env.RAILGRID_ACTIONS_WORKSPACE = 'workspace-from-env';
    let request;
    const client = createActionsClient({
      token: 'token',
      fetch: async (url, options) => {
    ...
    assert.equal(request.options.headers['X-Railgrid-Org'], 'org-from-env');
    assert.equal(request.options.headers['X-Railgrid-Workspace'], 'workspace-from-env');
  } finally {
    for (const [name, value] of Object.entries({
      RAILGRID_ACTIONS_BASE_URL: prior.base,
      RAILGRID_PROJECT: prior.project,
      RAILGRID_ACTIONS_ORG: prior.org,
      RAILGRID_ACTIONS_WORKSPACE: prior.workspace,
    })) {
